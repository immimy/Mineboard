...
export async function updateSession(
  ...
  // ❓🅿️ Checking whether it's a public route
  const pathname = request.nextUrl.pathname;
  const publicRouteRegexes = publicRoutes.map(createPublicRouteRegex);
  const isPublic = publicRouteRegexes.some((regex) => regex.test(pathname));
...
function createPublicRouteRegex(path: string) {
  // Use for '/' homepage
  if (path === '/') {
    // ✅ Matches ONLY exact path (+ query/hash)
    // ❌ No sub-paths
    return /^\/(\?[^#]*)?(#.*)?$/;
  }

  // Use for other paths (eg. '/newsletter')
  // Escape special regex chars in the path
  const escaped = path.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  // ✅ Allow: exact, trailing slash, sub-paths, query strings, hashes
